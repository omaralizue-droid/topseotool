...
export function ReportView({ report, isPublic = false }: ReportViewProps) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto p-6 md:p-10 bg-background text-foreground border border-border/60 rounded-xl shadow-lg print:border-none print:shadow-none print:p-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
        ...
        {!isPublic && (
          <div className="flex items-center gap-2 print:hidden">
            <Button size="sm" variant="outline" className="h-8 text-xs" onClick={handleCopyShareLink}>
              <Share2 className="h-3.5 w-3.5 mr-1.5" /> Share Link
            </Button>
            <Button size="sm" variant="brand" className="h-8 text-xs" onClick={handlePrint}>
              <Printer className="h-3.5 w-3.5 mr-1.5" /> Download / Print PDF
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 rounded-lg bg-card border border-border text-center">
          <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">SEO Health Score</p>
          <p className="text-3xl font-extrabold font-mono-nums text-emerald-600 dark:text-emerald-400">{report.seoScore}/100</p>
        </div>

        <div className="p-4 rounded-lg bg-card border border-border text-center">
          <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">AI Visibility Score</p>
          <p className="text-3xl font-extrabold font-mono-nums text-brand">{report.aiVisibilityScore}%</p>
        </div>

        <div className="p-4 rounded-lg bg-card border border-border text-center">
          <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">AI Mention Rate</p>
          <p className="text-3xl font-extrabold font-mono-nums text-amber-600 dark:text-amber-400">{report.mentionRate}%</p>
        </div>

        <div className="p-4 rounded-lg bg-card border border-border text-center">
          <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Citation Share</p>
          <p className="text-3xl font-extrabold font-mono-nums text-sky-600 dark:text-sky-400">{report.citationRate}%</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {report.technicalIssues.map((issue) => (
            <div key={issue.title} className="p-3 rounded-lg border border-border bg-card flex items-center justify-between text-xs">
              <span className="font-medium">{issue.title}</span>
              <Badge variant={issue.severity === "CRITICAL" ? "error" : "secondary"} className="text-[10px]">
                {issue.severity}
              </Badge>
            </div>
          ))}
